  ...
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. Create project_role enum and project_members table
    ...
    // 4. Migrate existing data (columns may already be dropped by synchronize)
    const hasProjectTeamId = await queryRunner.query(`
      SELECT 1 FROM information_schema.columns
      WHERE table_name = 'projects' AND column_name = 'team_id'
    `);
    const hasUserTeamId = await queryRunner.query(`
      SELECT 1 FROM information_schema.columns
      WHERE table_name = 'users' AND column_name = 'team_id'
    `);

    if (hasProjectTeamId.length > 0) {
      await queryRunner.query(`
        UPDATE teams t
        SET project_id = p.id
        FROM projects p
    ...
    if (hasUserTeamId.length > 0 && hasProjectTeamId.length > 0) {
      await queryRunner.query(`
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP TYPE IF EXISTS project_role`);
